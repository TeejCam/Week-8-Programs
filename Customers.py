...
def promptUser(customers, sortedByCompany, sortedByContact):
    while True:
        menu()
        option = input("Please type your choice as a number: ").strip().lower()

        if option == '1':
            displaySortedCustomers(customers, 'company')

        elif option == '2':
            displaySortedCustomers(customers,  'contact')

        elif option == '3':
            searchKey = input("Enter a company name or part of the name you'd like to find: ").strip()
            searchCustomers(customers, searchKey, 'CompanyName')

        elif option == '4':
            searchKey = input("Enter a contact name or part of the name you'd like to find: ").strip()
...
def displaySortedCustomers(sortedData, displayFormat):
    #sortedData = sortedCustomers(customers, keyName)
...
if __name__ == '__main__':
    ...
    #sort customers just once and store in variable for optimization
    sortedByCompany = sortedCustomers(customers, 'CompanyName')
    sortedByContact = sortedCustomers(customers, 'ContactName')

    promptUser(customers, sortedByCompany, sortedByContact)